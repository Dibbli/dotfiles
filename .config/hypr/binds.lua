...
hl.bind("Insert", hl.dsp.exec_cmd("hyprshot -m region --clipboard-only --freeze --silent"))
hl.bind("ALT + Insert", hl.dsp.exec_cmd("~/.config/hypr/scripts/screengif.sh"))
hl.bind("CTRL + Insert", hl.dsp.exec_cmd("hyprshot -m window --clipboard-only --silent"))

hl.bind(mainMod .. " + V", hl.dsp.exec_cmd("cliphist list | uwsm app -- fuzzel -d | cliphist decode | wl-copy"))
hl.bind(mainMod .. " + SHIFT + V", hl.dsp.exec_cmd("rm ~/.cache/cliphist/db"))
...
-- Move focus (Colemak-position hjkl)
...
for i = 1, 10 do
    local row = i % 10 -- workspace 10 sits on key 0
    local pad = numpadKeys[i]

    hl.bind(mainMod .. " + " .. row, hl.dsp.focus({ workspace = i }))
    hl.bind(mainMod .. " + SHIFT + " .. row, hl.dsp.window.move({ workspace = i }))
    hl.bind(mainMod .. " + " .. pad, hl.dsp.focus({ workspace = i }))
    hl.bind(mainMod .. " + SHIFT + " .. pad, hl.dsp.window.move({ workspace = i }))
end
