...
**How to apply:** before implementing, check the current branch. If it is already a ticket or feature branch (not `main`/`master`), work on it as-is. If on `main`, ask which branch to use or whether to create one, rather than inventing a `feat/` name. The user handles branch creation; the AI coding agent handles implementation within the branch they provide. Pairs with any commits-workflow rule (user handles commits).
